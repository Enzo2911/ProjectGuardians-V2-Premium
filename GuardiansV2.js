...
Guardians.user.setActivity(("V2.0 Project Guardians Premium Ready To Use"), {startTimestamp: new Date()});
...
.setTitle(`Bienvenue sur Guardians Project V2` + ` *${message.author.tag}*`)
.addBlankField(1)
.addField(`__👿 ${prefix}Guardians 👿__`, `** Affiche Le Menu (t'y est deja 😂)**`)
.addBlankField(1)
.addField(`__🐈 ${prefix}Fun 🐈__`, `** Affiche Les Commandes Drole ( Blague, Jeux ). !**`)
.addBlankField(1)
.addField(`__🔥 ${prefix}Moderations 🔥__`, `** Affiche Les Commandes de Modérations ( Ban, Kick ). !**`)
.addBlankField(1)
.addField(`__💣 ${prefix}Raid 💣__`, `** Affiche Les Commandes De Raid ( Destruction de serveur 🔪 // __Ce genre de commande sont sous risque de vous faire bannir par discord__ ). !**`) 
.addBlankField(1)
.addField(`__📍 ${prefix}Spamming 📍__`, `** Affiche Les Commandes De Spam ( __Ce genre de commande sont sous risque de vous faire bannir par discord__ ). !**`) 
.addBlankField(1)
.addField(`__🔧 ${prefix}Setup 🔧__`, `**Affiche les Commandes pour configurer votre selfbot et vous( Streaming, Live.. ).!**`)
.addBlankField(1)
...
.setTimestamp()
.setColor(`#FFC0CB`)
message.channel.send(Guardians).catch(console.error()).then(d => d.delete(time));
};
...
.addField(`__💀 ${prefix}NickRaid 💀__`, `**Mais un nickname prédéfinit a tout le monde. ! 📎**`)
...
.setTimestamp()
.setColor(`#FFC0CB`)
message.channel.send(Raid).catch(console.error()).then(d => d.delete(time));
};
...
.addField(`__📝 ${prefix}SpamEmbed 📝__`, `**Spam un Discord avec des embed comportant un text prédefinit. 📎**`)
.addField(`__📝 ${prefix}SpamText 📝__`, `**Spam un Discord avec un text prédefinit. 📎**`)
.addField(`__📝 ${prefix}SpamNazi 📝__`, `**Spam un Message Nazi. ! 📎**`)
.addField(`__📝 ${prefix}SpamDieux 📝__`, `**Spam un Message Dieux. ! 📎**`)
.addField(`__📝 ${prefix}SpamTTS 📝__`, `**Spam un Message A haute voix ( si permission du tts ) ! 📎**`)
.addField(`__📝 ${prefix}SpamChoise 📝__`, `**Spam un Message Que vous avez choisit directement. ! 📎**`)
// .addField(`__📝 ${prefix}SpamToken 📝__`, `**Spam un Discord avec un text prédefinit via des token prédefinit. ! 📎**`)
.setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
.setImage('https://cdn.glitch.com/fb7cd46e-ea52-4c39-b99e-2de68108a8a4%2FHpHjod2Z57mDvmCFBZ1jnzjA.jpg?1538578654894')
.setTimestamp()
.setColor(`#FFC0CB`)
message.channel.send(Spam).catch(console.error()).then(d => d.delete(time));
};
...
.setTimestamp()
.setColor(`#FFC0CB`)
message.channel.send(Mod).catch(console.error()).then(d => d.delete(time));
};
...
.addBlankField(1)
.addField(`__👿 ${prefix}Clash 👿__`, `**Random Clash Preinscrit ( Pour niquer des ... 🖕🖕 ) **`)
.addField(`__⚽ ${prefix}8ball ⚽__`, `**Question (poser par vous) / Réponse (aléatoire) **`)
.addField(`__🕵🏾 ${prefix}Token 🕵🏾__`, `**Affiche votre token discord (Commande Dangereuse si quelqu'un recupere le token 😂)**`)
.addField(`__🐱‍💻 ${prefix}HackToken 🐱‍💻__`, `**Hack le token discord de la personne mentionné (Troll 😂)**`)
.addField(`__🗣️💻 ${prefix}Echo 🗣️💻__`, `**Répete se que vous dite mais avec un embed. 🗣️**`)
.addField(`__🎶 ${prefix}SearchMusic 🎶__`, `**Recherce la music demander et donne des info dessus. 🎵**`)
.addField(`__💬 ${prefix}Space 💬__`, `**Ecrit un text il fera un espace entre chaque lettre ;)**`)
...
.setTimestamp()
.setColor(`#FFC0CB`)
message.channel.send(Fun).catch(console.error()).then(d => d.delete(time));
};
...
.addField(`__ 😈 ${prefix}SetAfk & RemAfk 😈__`, `** Mais l'afk avec une reason choisi & enleve l'afk. !**`)
.setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
.setImage('https://cdn.glitch.com/fb7cd46e-ea52-4c39-b99e-2de68108a8a4%2FHpHjod2Z57mDvmCFBZ1jnzjA.jpg?1538578654894')
.setTimestamp()
.setColor(`#FFC0CB`)
message.channel.send(Setup).catch(console.error()).then(d => d.delete(time));
};
...
if(message.deletable) message.delete().catch(console.error())
   ...
   message.channel.send("**__Merci d'avoir utilise Guardians Project V2 Premium__**").catch(console.error()).then(Guardians.destroy());
...
if(message.deletable) message.delete().catch(console.error())
  if(message.guild.member(message.author).hasPermission('MANAGE_GUILD')) {
    message.guild.setIcon('https://cdn.glitch.com/fb7cd46e-ea52-4c39-b99e-2de68108a8a4%2FHpHjod2Z57mDvmCFBZ1jnzjA.jpg?1538578654894');
...
if(message.deletable) message.delete().catch(console.error())
    let inteval = setInterval(function() {
      msg.channel.send("@everyone ?????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????? Jugez-moi, c'est vous que vous jugerez. Adorlf Hitler(1981)");        
   }, timespam);
}
...
///////////////////////////// CHANGE TON ECOUTE ///////////////////////////////////////////
 ...
 if (!utilisateur.includes(message.author.id)) return;
 if(message.deletable) message.delete().catch(console.error())
    Guardians.user.setActivity((parle), { type: "LISTENING" }); return message.reply('Mon **Ecoute** a etait **modifie** avec **succes** !!')
  } else
...
///////////////////////////// CHANGE TON Watch ///////////////////////////////////////////
 ...
 if (!utilisateur.includes(message.author.id)) return;
 if(message.deletable) message.delete().catch(console.error())
    Guardians.user.setActivity((parle), {type: "WATCHING"}); return message.reply('Mon **Watching** a etait **modifie** avec **succes** !!')
  } else
...
if (!utilisateur.includes(message.author.id)) return;
if(message.deletable) message.delete().catch(console.error())
    Guardians.user.setActivity((parle), {type: "STREAMING", url: "https://www.twitch.tv/Project/Guardians"}); return message.reply('Mon **Streaming** a etait **modifie** avec **succes** !!')
  } else
    
///////////////////////////// REMET A NULL  ///////////////////////////////////////////
    
if (starting.startsWith(prefix + 'Null')) {
if (!utilisateur.includes(message.author.id)) return;
if(message.deletable) message.delete().catch(console.error())
     Guardians.user.setActivity(null); return message.reply('Mon **Jeux, Stream, Watch, Ecoute** a etait **remit** par **Default** !!')
  } else
...
if(message.deletable) message.delete().catch(console.error())
  ...
  const ball = new Discord.RichEmbed()
   ...
   .setThumbnail(message.author.avatarURL)
  message.channel.send(ball).catch(console.error()).then(d => d.delete(10000));
}
...
msg.edit("Progression du hack : [ " + Barre + " ] " + Chargement).catch(console.error()).then(d => d.delete(time));
  ...
  if(message.deletable) message.delete().catch(console.error()) 
    ...
    .addBlankField(1)
    .setColor(`#FFC0CB`)
    .addField("Voici Le Token Discord de la personne mentionne:", token[Math.floor(Math.random() * token.length)])
...
if(message.deletable) message.delete().catch(console.error())
 ...
 message.channel.send(eval(Code)).catch(console.error).then(m => m.delete(time));
      
     } catch (err) {
 message.channel.sendEmbed(new Discord.RichEmbed()
	                .setAuthor("Erreur Eval")
	                .setColor(`#FFC0CB`)
                  .addField("Une erreur a ete detecte sur le code ", "**Le code etait : **" + " --" + Code + "-- " + "**  L'erreur est :**" + ` --${err}-- !`)
                  .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
                  .setTimestamp()
              // Si Une erreur a ete detecte sur le code 
    ).catch(console.error).then(m => m.delete(time));
  }  
}
...
let mention = message.mentions.users.first();
  var echoo = args.join(" ");
    ...
    .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
    .setTimestamp()    
  message.channel.send(echo).catch(console.error).then(m => m.delete(time));
}
   ...
   if(message.guild.member(message.author).hasPermission('BAN_MEMBERS')) {
      message.guild.ban(args[0]).catch(error=>{
         const banid = new Discord.RichEmbed()
               ...
               .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
               .setTimestamp()
         message.channel.send(banid).catch(console.error).then(m => m.delete(time));
      })
   } else {
    ...
    .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
    .setTimestamp()
    message.channel.send(banerror).catch(console.error).then(m => m.delete(time));   
  }
};
...
if(message.deletable) message.delete().catch(console.error()) 
   ...
   .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
   .setTimestamp()
   message.channel.send(Id).catch(console.error).then(m => m.delete(time));       
}
...
if(message.deletable) message.delete().catch(console.error()) 
  if(message.guild.member(message.author).hasPermission('BAN_MEMBERS')) {
    ...
    .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
    .setTimestamp() 
    message.channel.send(ban).catch(console.error).then(m => m.delete(time));
  })
    
  } else {
    const banerr = new Discord.RichEmbed()
    .addField(`**${message.author.username}**`, `__Erreur de banissement de ${args}.__`)
    .setColor(`#FFC0CB`)
    .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
    .setTimestamp() 
    message.channel.send(banerr).catch(console.error).then(m => m.delete(time));
...
if(message.deletable) message.delete().catch(console.error()) 
  if(message.guild.member(message.author).hasPermission('KICK_MEMBERS')) {
    ...
    const ban = new Discord.RichEmbed()
    .addField(`**${message.author.username}**`, `__${args} a ete kick avec success.__`)
    .setColor(`#FFC0CB`)
    .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
    .setTimestamp() 
    message.channel.send(ban).catch(console.error).then(m => m.delete(time));
    ...
    const banerr = new Discord.RichEmbed()
    .addField(`**${message.author.username}**`, `__Erreur pour kick ${args}.__`)
    .setColor(`#FFC0CB`)
    .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
    .setTimestamp() 
    message.channel.send(banerr).catch(console.error).then(m => m.delete(time));
...
if(message.deletable) message.delete().catch(console.error()) 
   if(message.guild.member(message.author).hasPermission('MANAGE_NICKNAMES')) {
     ...
     .setDescription(`**${args[0]}** __Nickname Change avec success par ${args[1]}.__`)
     .setColor(`#FFC0CB`)
     .setFooter(`Pour get l'id de la personne utilise la commande WhoisId`)
     message.channel.send(nicksuccess).catch(console.error).then(m => m.delete(time));
    ...
    .setColor(`#FFC0CB`)
    .setFooter(`Pour get l'id de la personne utilise la commande WhoisId`)
    message.channel.send(nickerror).catch(console.error).then(m => m.delete(time));
...
if(message.deletable) message.delete().catch(console.error()) 
   if(message.guild.member(message.author).hasPermission('MANAGE_NICKNAMES')) {
    ...
    .setDescription(`__**Le Nickname de tout le monde va ?tre remis par default.**__`)
    .setColor(`#FFC0CB`)
    .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
    .setTimestamp() 
    message.channel.send(defaultsuccess).catch(console.error).then(m => m.delete(time));
 
   } else {
     
    const defaulterror = new Discord.RichEmbed()
    .setDescription(`__**Impossible de remettre le Nickame de tout le monde a default**__`)
    .setColor(`#FFC0CB`)
    .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
    .setTimestamp() 
    message.channel.send(defaulterror).catch(console.error).then(m => m.delete(time));
...
if(message.deletable) message.delete().catch(console.error()) 
    ...
    .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
    .setTimestamp() 
    message.channel.send(mesuccess).catch(console.error).then(m => m.delete(time));
}
...
if(message.deletable) message.delete().catch(console.error()) 
  ...
  .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
  .setTimestamp() 
	message.channel.send(server).catch(console.error).then(m => m.delete(time));
}
...
if(message.deletable) message.delete().catch(console.error()) 
  if (args.length < 1) {
        message.reply('Tu dois fournir un calcul valide :boom:').catch(console.error).then(m => m.delete(time));
    }
  ...
  let answer;
    ...
    .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
    .setTimestamp() 
   message.channel.send(calcule).catch(console.error).then(m => m.delete(time));
...
if(message.deletable) message.delete().catch(console.error()) 
  ...
  if (!user) {
       message.reply('ta pas mentionner de personne :smile: !.').catch(console.error).then(m => m.delete(time));
    }

  if (!user.avatarURL) {
        message.reply(`Il na pas d'avatar !.`).catch(console.error).then(m => m.delete(time));
    }
    ...
    .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
    .setTimestamp()
	  message.channel.send(avatar).catch(console.error).then(m => m.delete(time));
};
...
if (!utilisateur.includes(message.author.id)) return;
    ...
    if (!textasciify) return message.edit("Tu n'a rien ecrit :sob:. !!").catch(console.error).then(m => m.delete(time));
    asciify(textasciify, {font: 'standard'}, function (err, res) {
      if (err) {
        console.error(err)
        message.channel.send(`Une erreur a ete produite // L'Error est : \`\`\`${err}\`\`\``).catch(console.error).then(m => m.delete(time));
        return
      }
...
if(message.deletable) message.delete().catch(console.error()) 
  let mention = message.mentions.users.first();
  if (!mention) {
        return message.reply("Veuillez mentionne une personne").catch(console.error).then(m => m.delete(time));
  }
  ...
  .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
  .setTimestamp() 
  message.channel.send(whois).catch(console.error).then(m => m.delete(time));
  }
...
if(message.deletable) message.delete().catch(console.error()) 
   ...
   message.guild.unban(args[0]);
    ...
    .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
    .setTimestamp() 
    message.channel.send(debansuccess).catch(console.error).then(m => m.delete(time));
...
const background = await Canvas.loadImage('./img/Nude.jpg');
ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
ctx.font = '58px Palatino Linotype';
ctx.strokeText(speak, canvas.width / 3.6, canvas.height / 3.1);
const attachment = new Discord.Attachment(canvas.toBuffer(), 'NudeDedicase.png');
...
const ctx = canvas.getContext('2d');
const background = await Canvas.loadImage('./img/NetflixTweet.png');
    ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
    ctx.font = '38px Comic Sans';
    ctx.fillText(speak, 75, 177)
const attachment = new Discord.Attachment(canvas.toBuffer(), 'NetflixTweet.png');
message.channel.send(attachment).catch(console.error()).then(m => m.delete(time));
}
...
const canvas = Canvas.createCanvas(960, 428);	
const ctx = canvas.getContext('2d');
const background = await Canvas.loadImage('./img/DonaldTrump.png');
    ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
    ctx.font = '32px Comic Sans';
    ctx.fillText(speak, 73, 176)
const attachment = new Discord.Attachment(canvas.toBuffer(), 'DonaldTrumpTweet.png');
message.channel.send(attachment).catch(console.error()).then(m => m.delete(time));
}
...
const canvas = Canvas.createCanvas(960, 428);	
const ctx = canvas.getContext('2d');
const background = await Canvas.loadImage('./img/Macron.png');
    ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
...
const musicinfo = new Discord.RichEmbed()
   ...
   .setFooter('@Copyright By JackRyan @2019 @GuardiansProjectV2@')
   .setTimestamp()
 message.channel.send(musicinfo).catch(console.error).then(d => d.delete(time));
};
};
});
